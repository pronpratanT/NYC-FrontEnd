...
const RejectPRModal: React.FC<RejectPRModalProps> = ({ open, onClose, onConfirm, prNo }) => {
    ...
    return (
        <div
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/70 backdrop-blur-sm transition-all duration-300"
        ...
        >
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl dark:shadow-slate-950/50 p-8 w-full max-w-lg relative border border-gray-100 dark:border-slate-600 transition-all duration-300 animate-in fade-in-0 zoom-in-95">
                ...
                <div className="flex flex-col items-center mb-6">
                    ...
                    <h2 className="text-2xl font-bold text-red-600 dark:text-red-400 mb-1">ปฏิเสธ <span className="text-gray-700 dark:text-slate-200">{prNo}</span></h2>
                    <span className="text-sm text-gray-500 dark:text-slate-400">กรุณาระบุเหตุผลในการปฏิเสธคำขอ</span>
                </div>
